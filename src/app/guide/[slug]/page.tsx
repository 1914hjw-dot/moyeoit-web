...
const GUIDE_ARTICLES_MAP: Record<string, ArticleDetailContent> = {
  'free-date-selection-guide': {
    ...
    category: '날짜 선택 모드',
    description: '방장이 기간을 지정하는 모드와 후보 날짜 없이 자유롭게 투표하는 자유 날짜 모드의 차이점과 모임별 맞춤 가이드를 공개합니다.',
    summary: '방장이 기간을 지정하는 모드와 후보 날짜 없이 자유롭게 투표하는 자유 날짜 모드의 비교 가이드.',
    date: '2026-08-08',
    readTime: '3분 읽기',
    content: {
      intro: '모여잇(Moyeoit)은 사용자들의 다양한 모임 성격에 맞춰 2가지 강력한 날짜 조율 모드([기간 내에서 선택]과 [자유 날짜 선택])를 제공합니다. 내 모임에 어떤 모드가 더 어울리는지 확인해보세요.',
      sections: [
        {
          heading: '1. 기간 내에서 선택 모드 (RANGE)',
          body: '방장이 미리 "8월 10일부터 20일 사이"처럼 특정 모임 기간이나 후보 주말을 한정해두고, 참여자들이 그 안에서 가능한 날짜를 투표하는 방식입니다. 일정 범위가 정해진 팀 회식이나 1박 2일 여행에 최적화되어 있습니다.',
        },
        {
          heading: '2. 자유 날짜 선택 모드 (FREE)',
          body: '방장이 후보 날짜나 요일을 미리 지정하지 않는 10초 초간단 방식입니다. 방을 생성한 후 참여자들이 캘린더에서 각자 참석 가능한 날짜를 자유롭게 클릭하여 제출합니다. 날짜 제한 없이 각자 스케줄을 수집해야 하는 오래된 친구 모임이나 동아리 모임에 강력히 추천합니다.',
        },
        {
          heading: '3. 전원 참석 가능 (ALL_AVAILABLE) 및 최다 가능 (MAX_AVAILABLE) 판정',
          body: '어떤 모드를 선택하더라도 모여잇 결정 엔진이 전체 참여자 수(N) 대비 참석 가능 비율을 실시간으로 분석합니다. 전원이 가능한 날짜가 있다면 [🎉 전원 참석 가능한 황금 날짜]를 1위로 추천하고, 동률이 발생하면 모든 공동 1위 날짜를 묶어 투명하게 표출합니다.',
        },
        {
          heading: '4. 방장의 최종 약속 날짜 확정 및 4단계 카톡 공유',
          body: '투표 결과가 모이면 방장은 추천 카드에서 [이 날짜로 모임 확정하기] 버튼을 누릅니다. 확정이 완료되면 파티 폭죽 카드와 함께 카카오톡 및 4단계 복구 공유 링크가 활성화됩니다.',
        },
      ],
      conclusion: '모임 성격에 따라 기간 지정 모드와 자유 날짜 모드를 유연하게 활용해보세요. 번거로운 카톡 투표 대신 10초 만에 황금 약속 날짜를 발견할 수 있습니다.',
    },
  },
  'fast-date-picker': {
