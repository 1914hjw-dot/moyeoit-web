...
export const CalendarSelector: React.FC<CalendarSelectorProps> = ({
  ...
  return (
    <div className="w-full sys-card p-4 sm:p-6 space-y-5 bg-white border-slate-200/80 shadow-sm">
      {/* 1. Date Selection Mode Tabs (RANGE vs FREE) */}
      {onChangeDateSelectionMode && (
      ...
      {/* FREE Mode Info Banner (Hides unnecessary scheduleType/weekday controls) */}
      {isFreeMode ? (
        <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200/80 space-y-1 text-xs text-emerald-950 shadow-xs">
          <div className="flex items-center gap-1.5 font-black text-emerald-950">
            <Globe className="w-4 h-4 text-emerald-600 shrink-0" />
            <span>자유 날짜 선택 모드 활성화됨</span>
          </div>
          <p className="text-[11px] text-emerald-800 leading-relaxed">
            방장이 후보 날짜나 요일을 미리 지정하지 않고 바로 방을 생성합니다. 모임 링크를 받은 참여자들이 캘린더에서 각자 가능한 날짜를 자유롭게 선택할 수 있습니다.
          </p>
        </div>
      ) : (
        /* RANGE Mode Controls (Schedule Type & Time Slots) */
        <>
          <div>
      ...
      {/* Calendar & Pre-selection (Rendered in RANGE mode, or optional preview in FREE mode) */}
      {!isFreeMode && (
        <>
